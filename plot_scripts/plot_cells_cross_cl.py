...
params_mom_path = 'data/params_moments_fiducial.txt'                   
...
fid_params = np.loadtxt(params_fid_path)[:nsims,1:]
r_fid = fid_params[:nsims,np.where(fid_names=='r_tensor')].flatten()
AL_fid = fid_params[:nsims,np.where(fid_names=='A_lens')].flatten()
with open(params_mom_path) as mom:
    mom_names = np.array(mom.readline().strip().split()[1:])
mom_params = np.loadtxt(params_mom_path)[:nsims,1:]
r_mom = mom_params[:nsims,np.where(mom_names=='r_tensor')].flatten()
AL_mom = mom_params[:nsims,np.where(mom_names=='A_lens')].flatten()

cmb_dls_fid = r_fid[:nsims,None]*cmb_tens[None,:] + \
                AL_fid[:nsims,None]*cmb_lens[None,:]
cmb_dls_mom = r_mom[:nsims,None]*cmb_tens[None,:] + \
                AL_mom[:nsims,None]*cmb_lens[None,:]
...
for il,l in enumerate(ls):
    if l in list(ells.astype('int')):
        cmb_dls_fid_ells.append(cmb_dls_fid[:nsims,il])
        cmb_dls_mom_ells.append(cmb_dls_mom[:nsims,il])    
cmb_dls_fid_ells = np.asarray(cmb_dls_fid_ells).T
cmb_dls_mom_ells = np.asarray(cmb_dls_mom_ells).T

print('shape', cmb_tens.shape)

dells_err = np.load(dells_path)['el']/np.sqrt(nsims)
dells_mom = np.load(dells_model_mom_path)['dl']
residual_dells_fid = np.mean(np.load(dells_path)['dl'][:nsims] - \
                             np.load(dells_model_fid_path)['dl'][:nsims] + \
                             cmb_dls_fid_ells[:nsims,:,None,None], axis=0)
residual_dells_mom = np.mean(np.load(dells_path)['dl'][:nsims] - \
                             np.load(dells_model_mom_path)['dl'][:nsims] + \
                             cmb_dls_mom_ells[:nsims,:,None,None], axis=0)

####
# 3: Make plot (D_ells).
fig, axes = get_triangle_axes()
...
for i1 in range(6):
    for i2 in range(i1,6):
        ...
        # CL-moments
        ax.errorbar(ells, residual_dells_mom[:,i1,i2], yerr=dells_err[:,i1,i2], fmt='r.', elinewidth=.5)
        ...
        if (i1==1) or (i1==3) or (i1==5):
            ax.set_yticklabels([])
plt.suptitle(f'Average over {nsims} sim(s), forgrounds dmsm / noise inhom. gop')
...
for i1 in range(6):
    for i2 in range(i1,6):
        ax = axes[i2, i1]
        ax.plot(ls, dl2cl*cmb_lens, 'k-', label=r'$(r=0,\,A_{\rm lens}=1)$ model')
        
        # CL-fiducial
        ax.errorbar(ells, dell2cell*residual_dells_fid[:,i1,i2], yerr=dell2cell*dells_err[:,i1,i2], fmt='b.', elinewidth=.5, label=r'data $-$ foregrounds ($C_{\ell}$-fiducial)')
        ...
        ax.scatter(ells[high_fid], np.full(len(ells[high_fid]), ylims[i1][1]), c='b', marker='^', s=30)
        ax.scatter(ells[low_fid], np.full(len(ells[low_fid]), ylims[i1][0]), c='b', marker='v', s=30)
        ax.plot(ls, dl2cl*np.mean(cmb_dls_fid, axis=0), 'b--', label=r'CMB best-fit model ($C_{\ell}$-fiducial)')
        
        # CL-moments
        ax.errorbar(ells, dell2cell*residual_dells_mom[:,i1,i2], yerr=dell2cell*dells_err[:,i1,i2], fmt='r.', elinewidth=.5, label=r'data $-$ foregrounds ($C_{\ell}$-moments)')
        ...
        ax.scatter(ells[high_mom], np.full(len(ells[high_mom]), ylims[i1][1]), c='r', marker='^', s=20)
        ax.scatter(ells[low_mom], np.full(len(ells[low_mom]), ylims[i1][0]), c='r', marker='v', s=20)
        ax.plot(ls, dl2cl*np.mean(cmb_dls_mom, axis=0), 'r--', label=r'CMB best-fit model ($C_{\ell}$-moments)')
        ...
        if i1 == 5 and i2 == 5:
            ...
            fig.legend(handles,labels, fontsize=15, bbox_to_anchor=(0.85, 0.85))
#plt.suptitle(f'Average over {nsims} sim(s), foregrounds dmsm / noise inhom. gop')
